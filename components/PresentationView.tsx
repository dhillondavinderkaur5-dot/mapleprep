...
import { updateUserImageUsage } from '../services/auth';
import { 
  ...
  UploadCloud, FileVideo, FileImage, Loader2, Save, Check, Minus, Undo
} from 'lucide-react';
...
interface PresentationViewProps {
  ...
  onSaveLesson?: (plan: LessonPlan) => Promise<void>;
...
export const PresentationView: React.FC<PresentationViewProps> = ({ 
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      if (userId) {
        await updateUserImageUsage(userId);
        onImageGenerated?.();
      }
    } catch (e) {
  ...
  const handleSlideMouseMove = (e: React.MouseEvent) => {
    if (!isEditMode || !slideContainerRef.current) return;

    if (resizingElementId) {
      const deltaX = e.clientX - initialResizePos.x;
      const updatedSlides = [...lessonPlan.slides];
      const elements = [...(updatedSlides[currentSlideIdx].customElements || [])];
      const elementIdx = elements.findIndex(el => el.id === resizingElementId);
      
      if (elementIdx !== -1) {
        // Sensitivity: 200px of drag = 1.0 scale change
        const newScale = Math.max(0.2, Math.min(4, initialScale + (deltaX / 200)));
        elements[elementIdx] = { ...elements[elementIdx], scale: newScale };
  ...
  const handleScaleElement = (e: React.MouseEvent, id: string, delta: number) => {
    e.stopPropagation();
    const updatedSlides = [...lessonPlan.slides];
    const elements = [...(updatedSlides[currentSlideIdx].customElements || [])];
    const elementIdx = elements.findIndex(el => el.id === id);
    
    if (elementIdx !== -1) {
      const currentScale = elements[elementIdx].scale || 1;
    ...
    <div ref={containerRef} className={`flex flex-col overflow-hidden relative ${isFullscreen ? 'h-screen w-screen bg-slate-900' : 'h-full rounded-2xl shadow-2xl bg-slate-100'} animate-fadeIn`} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
          ...
          <div className="p-4 bg-white border-b flex justify-between items-center shrink-0">
              ...
              <div className="flex items-center gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border-2 transition-all ${penColor === c.color ? 'scale-125 border-slate-900 shadow-md' : 'border-transparent hover:scale-110'}`}
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                className={`p-2 rounded-xl transition-all ${undoStack.length === 0 ? 'text-slate-200' : 'bg-slate-100 text-slate-600 hover:text-blue-600 hover:bg-blue-50'}`}
                title="Undo"
              >
                <Undo className="w-5 h-5" />
              </button>
              <button onClick={clearWhiteboard} className="p-2 bg-slate-100 text-slate-400 hover:text-red-600 rounded-xl" title="Clear All"><Eraser className="w-5 h-5" /></button>
      ...
      <div className={`p-4 flex justify-between items-center bg-white border-b z-30 shrink-0 ${isFullscreen ? 'sticky top-0 left-0 right-0 border-none bg-white/90 backdrop-blur shadow-lg' : ''}`}>
        <div className="flex gap-2">
          <button onClick={onReset} className="p-2 rounded-lg hover:bg-slate-100" title="Exit to Dashboard"><X className="w-5 h-5"/></button>
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            disabled={isSaving}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border-2 transition-all ${saveSuccess ? 'bg-green-600 text-white border-green-600' : 'bg-white border-slate-200 text-slate-700 hover:border-green-500 hover:text-green-600'}`}
          >
            {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : saveSuccess ? <Check className="w-4 h-4" /> : <Save className="w-4 h-4" />}
            {isSaving ? 'Saving...' : saveSuccess ? 'Saved to Library' : 'Save to Library'}
          </button>
          <button onClick={() => setIsEditMode(!isEditMode)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border-2 transition-all ${isEditMode ? 'bg-blue-600 text-white border-blue-600 shadow-md scale-105' : 'bg-white border-slate-100 text-slate-500 hover:border-blue-500'}`}>
             {isEditMode ? <MousePointer2 className="w-4 h-4"/> : <Edit3 className="w-4 h-4"/>} {isEditMode ? 'Finish Editing' : 'Edit Tools'}
          </button>
            ...
            <div className={`w-full h-full flex gap-4 sm:gap-8 items-stretch relative min-h-0 ${isFullscreen ? 'max-w-none' : 'max-w-6xl'}`} ref={slideContainerRef} onMouseMove={handleSlideMouseMove}>
                {currentSlide.customElements?.map((el) => (
                  ...
                  >
                    {isEditMode && (
                      <>
                        <div className="absolute -top-4 -right-4 flex gap-1 opacity-100 sm:opacity-0 group-hover/el:opacity-100 transition-opacity z-[60]">
                          <button 
                            onMouseDown={(e) => e.stopPropagation()}
                            onClick={(e) => handleScaleElement(e, el.id, 0.1)}
                            className="p-2 bg-blue-600 text-white rounded-full shadow-lg hover:scale-110 active:scale-90"
                            title="Increase Size"
                          >
                          ...
                          <button 
                            onMouseDown={(e) => e.stopPropagation()}
                            onClick={(e) => handleScaleElement(e, el.id, -0.1)}
                            className="p-2 bg-amber-600 text-white rounded-full shadow-lg hover:scale-110 active:scale-90"
                            title="Decrease Size"
                          >
                          ...
                          <button 
                            ...
                            className="p-2 bg-red-600 text-white rounded-full shadow-lg hover:scale-110 active:scale-90"
                            title="Remove"
                          >
                            <X className="w-3 h-3"/>
                          </button>
                        ...
                        <div 
                          ...
                          className="absolute -bottom-3 -right-3 w-8 h-8 bg-blue-600 text-white rounded-full cursor-nwse-resize flex items-center justify-center shadow-xl z-[70] hover:scale-110 active:scale-95 transition-transform"
                ...
                {!showQuizResults ? (
                  ...
                  <div className="w-full max-w-xl bg-white rounded-[2rem] lg:rounded-[3rem] p-10 lg:p-16 shadow-2xl text-center border-4 border-slate-100 animate-fadeIn">
                     ...
                     <div className="space-y-3 lg:space-y-4">
                        <button onClick={resetQuiz} className="w-full py-4 lg:py-5 bg-red-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-red-700 transition-all flex items-center justify-center gap-3 shadow-xl">
                          <RefreshCw className="w-5 h-5" /> Restart Quiz
                        </button>
                        <button onClick={() => setActiveTab('slides')} className="w-full py-4 lg:py-5 bg-slate-100 text-slate-600 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
                          Back to Slides
                        </button>
                     </div>
                  </div>
      ...
      <div className={`p-4 bg-white border-t flex justify-between items-center px-6 sm:px-10 shrink-0 z-30 ${isFullscreen ? 'sticky bottom-0 left-0 right-0 border-none bg-white shadow-[0_-10px_30px_rgba(0,0,0,0.1)]' : ''}`}>
         <button onClick={() => setShowNotes(!showNotes)} className={`px-4 sm:px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-2 transition-all active:scale-95 shadow-md ${showNotes ? 'bg-red-600 text-white border-red-600' : 'bg-white border-2 border-slate-200 text-slate-600 hover:border-red-600 hover:text-red-600'}`}>
            ...
            {showNotes ? 'Hide Guide' : 'Show Guide'}
         </button>
